...
import { UsegetTransfers } from "../hooks/queries/get/Get";
import { UseRestituteTransfer } from "../hooks/queries/post/Post";
const transactions: {
  id: number;
  name: string;
  href: string;
...
export const BackOfficeTransfertComponent = () => {
  const queryClient = useQueryClient();
  const getToServeTranfers = UsegetTransfers();
  const [transfers, setTransfers] = useState<any[]>();
  useEffect(() => {
    setTransfers(getToServeTranfers.data);
  }, [getToServeTranfers.data]);
  const onRestitute = (id: string) => {
    const useRestitute = UseRestituteTransfer(id);
    if (useRestitute.isSuccess) {
      queryClient.invalidateQueries(
        "get:/ws-backoffice/restitution/getTransfer"
      );
    }
  };
  return (
                            ...
                            case "A_servir":
                              return (
                                <>
                                  {" "}
                                  <a
                                    onClick={() => onRestitute(transaction.id)}
